...
public class MinTemp {
	...
	public WeatherInfo minTemp(String zip,String city)
	{
		...
		try {
			...
			String SQLString = "SELECT min(temp),u_temperature,zip,city from weather_info where zip = '" + zip + "';";
			if(zip.isEmpty())
			{
				SQLString = "SELECT min(temp),u_temperature,zip,city from weather_info where city = '" + city + "';";
			}
			...
			System.out.println("Min Temp @"+ wi.getCity()+": "+wi.getTemp()+ " " + wi.getU_temperature());
